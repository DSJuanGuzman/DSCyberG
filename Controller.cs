...
{
    /// <summary>
    /// 控制器类，用于管理CyberGear控制系统中的CAN总线通信。Gestion de comunicacion por medio de Bus CAN con el sistema de control de CyberGear
    /// </summary>
    /// <remarks>
    /// 该类提供了一系列方法，包括发送和接收CAN消息，解析接收到的消息，Envio,Recepcion y Analisis de mensajes CAN
    /// 以及处理与主控制器和电机相关的逻辑。
    /// </remarks>
    /// Podriamos utilizar .Net IoT Desde un dispostivo IoT para acceder directamente al puerto de comunicacion CAN
    internal class Controller
    {
        ...
        {
            // 初始化控制器 Inicializa El Constructor
        ...
        private class CmdModes
        {
            ...
            public const uint BAUD_RATE_MODIFICATION = 22; //modificar el rango de Baudios de la comunicacion, tener mucha precaucion al modificarlo puede dejar el motor inutilizable

        }
        // 枚举运行模式
        public enum RunModes
        {
            //Modos del manual del CyberGear
            CONTROL_MODE = 0,    // 运控模式
        ...
        public static class ParameterList
        {
            ...
            public const uint RunMode = 0x7005; // 运控模式：0-位置模式；1-速度模式；2-电流模式 Modo de control de funcionamiento: 0: Operation control mode
            //1: Position mode
            //2: Speed mode
            //3: Current mode
            ...
            public const uint CurKp = 0x7010; // 电流的Kp（float类型，默认值0.125）ganancia proporcional
            public const uint CurKi = 0x7011; // 电流的Ki（float类型，默认值0.0158）ganancia derivada
        ...
        /// <summary>
        /// 向指定索引处写入单个浮点参数值。//asignacion de un unico valor en el indice ya especificado
        /// </summary>
        ...
        public void SendMotorControlCommand(float torque, float target_angle, float target_velocity, float Kp, float Kd)
        {
            ...
            //Kp: 比例增益。= Ganancia Proporcional
            //Kd: 导数增益。= Ganancia Derivada

            //生成29位的仲裁ID的组成部分
            //uint cmd_mode = CmdModes.MOTOR_CONTROL;
            uint torque_mapped = Calculate.FToU(torque, -12.0, 12.0);//Float to Uint
            uint data2 = torque_mapped;//data2为力矩值
            // 计算仲裁ID，
            uint arbitrationId = (CmdModes.MOTOR_CONTROL << 24) | (data2 << 8) | MotorCANID; //Encabezado de la peticion

            // 生成数据区1
            uint target_angle_mapped = Calculate.FToU(target_angle, -4 * Math.PI, 4 * Math.PI);//目标角度 Angulo Objetivo
            uint target_velocity_mapped = Calculate.FToU(target_velocity, -30.0F, 30.0F);//目标速度 Velocidad Objetivo
            uint Kp_mapped = Calculate.FToU(Kp, 0.0F, 500.0F);//比例增益 Ganancia Proporcional
            uint Kd_mapped = Calculate.FToU(Kd, 0.0F, 5.0F);//微分增益 Ganancia Diferencial

            //组合为一个8个字节的data1-- datos a 8bytes
