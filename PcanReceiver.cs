...
namespace CyberGear_Control_.NET
{
    internal class PcanReceiver
    {
        ...
        public bool Start()
        {
            ...
            {
                // 在Windows操作系统上，直接设置接收事件 Recepcion CAN Windows
            ...
            {
                // 在非Windows操作系统上，获取接收事件句柄并进行设置 // Recepcion CAN otros Sistemas
                uint eventHandle;
        ...
        private void ReceiveThread()
        {
            while (isRunning)
            {
                ...
                if (receiveEvent.WaitOne(50))
                {
                    ...
                    // 读取并处理接收缓冲区中的所有CAN消息 Lectura y procesamietno de los mensajes CAN en el buffer
                    while (Api.Read(channel, out canMessage, out canTimestamp) == PcanStatus.OK)
                    {
                        // 处理接收到的CAN消息
                        //Procesa el mensaje recibido
                        Console.WriteLine($"接收到的消息(Mensaje Recibido): ID=0x{canMessage.ID:X} 数据(Data)={BitConverter.ToString(canMessage.Data)}");
                        Console.WriteLine($"接收到的消息的时间戳(Tiempo Del Mensaje): {canTimestamp}");
